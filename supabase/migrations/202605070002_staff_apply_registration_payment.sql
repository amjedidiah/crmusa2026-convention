-- Atomic payment insert + registration aggregate update (staff / imports only)
...
comment on function public.staff_apply_registration_payment is
  'Inserts registration_payments and updates registrations in one transaction. Called only from trusted server code (service role).';
